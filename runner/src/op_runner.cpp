...
bool OpRunner::Init()
{
    for (size_t i = 0; i < numInputs_; ++i) {
        ...
        if (aclrtMalloc(&devMem, size, ACL_MEM_MALLOC_HUGE_FIRST) != ACL_SUCCESS) {
            ERROR_LOG("Malloc device memory for device input[%zu] failed", i);
            return false;
        }
        ...
        if (g_isDevice) {
            if (aclrtMalloc(&hostMem, size, ACL_MEM_MALLOC_HUGE_FIRST) != ACL_SUCCESS) {
                ERROR_LOG("Malloc device memory for host input[%zu] failed", i);
                return false;
            }
        } else {
    ...
    for (size_t i = 0; i < numOutputs_; ++i) {
        ...
        void *devMem = nullptr;
        if (aclrtMalloc(&devMem, size, ACL_MEM_MALLOC_HUGE_FIRST) != ACL_SUCCESS) {
            ERROR_LOG("Malloc device memory for device output[%zu] failed", i);
            return false;
        }
        ...
        if (g_isDevice) {
            if (aclrtMalloc(&hostOutput, size, ACL_MEM_MALLOC_HUGE_FIRST) != ACL_SUCCESS) {
                ERROR_LOG("Malloc device memory for host output[%zu] failed", i);
                return false;
            }
        } else {
        ...
        if (hostOutput == nullptr) {
            ERROR_LOG("Malloc memory for output[%zu] failed", i);
            return false;
        }
...
bool OpRunner::RunOp()
{
    for (size_t i = 0; i < numInputs_; ++i) {
        ...
        INFO_LOG("Copy input[%zu] successfully", i);
    }

    aclrtStream stream = nullptr;
    aclError code = aclrtCreateStream(&stream);
    if (code != ACL_SUCCESS) {
        ERROR_LOG("Create stream failed, errorCode[%d]",
    ...
    INFO_LOG("Execute %s successfully", opDesc_->opType.c_str());
