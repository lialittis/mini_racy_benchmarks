...
`runner/` 只完成一次设备执行。它不选择 case、不解析 sanitizer 日志，也不创建实验编号。模型、输入、输出、ACL 配置和设备号均由命令行传入。

`tools/bench.py` 是实验控制面，负责确定性数据、runner/mssanitizer 命令、数值比较和 JSON/Markdown 报告。shell 脚本只负责加载 CANN 环境并提供短命令入口。
...
新算子应创建自己的 `benchmarks/<operator>/`，包括 manifest、配置、内核和模型；同时增加一个只负责该算子张量描述的 runner target。共享 ACL 内存与文件逻辑继续放在 `runner/`，算子特有的参考计算和数据生成接入 `tools/bench.py` 的 workload 层，不应复制 sanitizer 编排代码。

## 结果判定

实验通过需要同时满足：runner 返回码为零、实际 hazard 类型包含 manifest 的 required 集合且不超出 allowed 集合、数值结果满足 manifest 的 match、mismatch 或 either 策略。hazard 数量保留在报告中观察，但不作为硬编码门槛，以允许 CANN instrumentation 版本变化。
